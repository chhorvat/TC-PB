...
int_chl_vanu = squeeze(nanmean(nanmean(bsxfun(@times,vanu_peak_locs,chl_infilled),1),2));
int_chl_vanu_SEAWIFS = squeeze(nanmean(nanmean(bsxfun(@times,vanu_peak_locs,chl_SEAWIFS_infilled),1),2));
int_chl_vanu_VIIRS = squeeze(nanmean(nanmean(bsxfun(@times,vanu_peak_locs,chl_VIIRS_infilled),1),2));
int_chl_vanu_HIMA = squeeze(nanmean(nanmean(bsxfun(@times,vanu_peak_locs,chl_HIMA_infilled),1),2));
...
Ax{1} = subplot('position',[0.04 0.55 0.25 0.4]);
...
Ax{2} = subplot('position',[0.04 0.05 0.25 0.4]);
...
Ax{3} = subplot('position',[0.365 0.55 0.25 0.4]);
worldmap(lat_lim_Oma,lon_lim_Oma);
...
%
Ax{5} = subplot('position',[.39 .075 .55 .4]);
...
for i = 1:length(Ax)
    ...
    if i~=5
        ...
        colorbar(Ax{i},'position',[pos(1)+pos(3)+.01 pos(2)+.025 .015 .35]);
        set(Ax{i},'fontsize',9);
    end
...
saveas(gcf,'/Users/chorvat/Dropbox (Brown)/Apps/Overleaf/Cyclones-Phytoplankton/Figures/Fig-1.pdf');
saveas(gcf,'Fig-1.pdf'); 
saveas(gcf,'/Users/chorvat/Dropbox (Brown)/Apps/Overleaf/Cyclones-Phytoplankton/Figures/Fig-1.jpeg');
